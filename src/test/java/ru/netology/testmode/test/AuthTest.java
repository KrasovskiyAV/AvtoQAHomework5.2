...
class AuthTest {
    ...
    @Test
    @DisplayName("Should show error if login empty")
    void ShouldShowErrorIfLoginEmpty() {
        $("[data-test-id='password'] input").setValue(getRandomPassword());
        $("button.button").click();
        $(".input_invalid[data-test-id='login'] .input__sub").shouldHave(text("Поле обязательно для заполнения")).shouldBe(visible);
    }

    @Test
    @DisplayName("Should show error if password empty")
    void ShouldShowErrorIfPasswordEmpty() {
        $("[data-test-id='login'] input").setValue(getRandomLogin());
        $("button.button").click();
        $(".input_invalid[data-test-id='password'] .input__sub").shouldHave(text("Поле обязательно для заполнения")).shouldBe(visible);
    }
}
